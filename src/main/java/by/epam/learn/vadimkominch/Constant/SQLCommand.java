...
public class SQLCommand {
    ...
    public static final String SELECT_USER_FOR_LOGIN = "SELECT confidentials.User_id AS id,confidentials.Login AS login,confidentials.Email AS email,confidentials.Password AS password,Users.NickName AS nickname,Users.FirstName AS firstname,Users.SecondName AS lastname from `Users` join `confidentials` on Users.User_id = confidentials.User_id WHERE confidentials.Email like ? OR confidentials.Login like ?;";
    ...
    public static final String UPDATE_USER = "UPDATE `Users` SET Login = ?  WHERE ?";
    public static final String RAISE_USER_ROLE = "UPDATE `Users` SET WHERE FirstName=? AND LastName=?";

    //Advertisment commands
    public static final String GET_AMOUNT_OF_ADVERTISMENTS = "SELECT COUNT(*) AS `count` from `paragraphs`";
    public static final String GET_AMOUNT_OF_ADVERTISMENTS_BY_USER = "SELECT COUNT(*) AS `count` from `paragraphs` WHERE author_id = ?";
    ...
    public static final String DELETE_ADVERTISMENT_NAME = "DELETE FROM `Paragraphs` WHERE paragraph_id = ?";//deleting by nickname
    ...
    public static final String UPDATE_ADVERTISMENT_NAME_AND_CATEGORY = "UPDATE `Paragraphs` SET `Parapgraph_name` = ?,`Category` = ? WHERE  paragraph_id = ? ";
...
//    public static final String GET_PAGE_OF_ADVERTISMENTS = "SELECT *  FROM    ( SELECT    ROW_NUMBER() OVER ( ORDER BY author_id ) AS RowNum, paragraphs.Category AS category, paragraphs.Parapgraph_name AS name, paragraphs.author_id AS author_id,paragraph_articles.Article_text AS text, paragraphs.paragraph_id AS paragraph_id,paragraph_articles.creation_date AS creation_date FROM  `paragraphs` JOIN `paragraph_articles` ON paragraphs.paragraph_id = paragraph_articles.paragraph_id WHERE paragraphs.Paragraph_id < 10) AS RowConstrainedResult WHERE   RowNum >= ?  AND RowNum < ? ORDER BY RowNum;";
    ...
    public static final String GET_PAGE_OF_ADVERTISMENTS_BY_USER = "SELECT *  \n" +
            ...
            "paragraphs.Category AS category, \n" +
            "paragraphs.Parapgraph_name AS name,\n" +
            ...
            "paragraphs.author_id AS author_id\n" +
            "FROM  `paragraphs` JOIN `paragraph_articles` ON paragraphs.paragraph_id = paragraph_articles.paragraph_id \n" +
            "JOIN `users` on paragraphs.author_id = users.User_id) \n" +
